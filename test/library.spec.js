...
import {erf, erfc, numInt, mounding} from '../lib/library.js';
...
describe('Given the erf(c)-function', () => {
    ...
    values.forEach( v => {
        ...
        it('calculating erf(x) with x=' + x + ' should return ' + expected, () => {
            expect(erf(x, decimals)).to.be.equal(expected);
        });
    });
    ...
    values.forEach( v => {
        ...
        it('calculating erfc(x) with x=' + x + ' should return ' + expected, () => {
            expect(erfc(x, decimals)).to.be.equal(expected);
        });
    });
});
...
describe('Given the numericallyIntegrate-function', () => {
    ...
    values.forEach( v => {
        ...
        it('calculating with a=' + a + ', b=' + b + ', dx=' + dx + ', should return ' + expected, () => {
            expect(numInt(a, b, dx, f).toFixed(decimals)).to.be.equal(expected);
        });
    });
});
...
describe('Given the mounding hMax-function', () => {
    ...
    values.forEach( v => {
        ...
        it('calculating hMax', () => {
            expect(mounding.calculateMax(w, L, W, hi, Sy, K, t).toFixed(decimals)).to.be.equal(expected);
        });
    });
});
